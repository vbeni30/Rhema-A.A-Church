      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {sermonSeries.map((series, index) => (
            <div
              key={index}
              className={`bg-gray-50 rounded-lg shadow-md overflow-hidden ${series.current ? "ring-2 ring-[#9B2B3A]" : ""}`}
            >
              <div className="relative h-48">
                <Image src={series.image || "/placeholder.svg"} alt={series.title} fill className="object-cover" />
                {series.current && (
                  <div className="absolute top-4 right-4 bg-[#9B2B3A] text-white px-3 py-1 rounded-full text-sm font-semibold">
                    Current Series
                  </div>
                )}
              </div>
        ...
        <div className="mt-12 text-center">
          <Button className="bg-[#9B2B3A] hover:bg-[#8a2533] text-white px-8 py-6 text-lg">
